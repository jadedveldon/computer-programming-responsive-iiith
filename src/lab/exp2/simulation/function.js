window.view = {
	...
	activateClicks: function() {
		...
		this.addClickEvent('imageRTTriangle', function() { view.showRTTriangleInputs() })
		this.addClickEvent('okRTTriangle', function() { view.validateRTTriangleInputs() })
	...
	showSquareInputs: function() {
		this.hideQuestionBlock('questionRTTriangle', 'imageRTTriangle')
	...
	showRTTriangleInputs: function() {
		this.hideQuestionBlock('questionSquare', 'imageSquare')
		this.hideQuestionBlock('questionCircle', 'imageCircle')
		this.hideQuestionBlock('questionTriangle', 'imageTriangle')
		this.showQuestionBlock('questionRTTriangle', 'imageRTTriangle')
        this.hideInstructions()
		this.unCheckRadioButtons()
	},
	...
	approveRTTriangleInputs: function() {
		this.displayFunctionForRTTriangle()
        this.showInstructions()
		document.getElementById('questionRTTriangle').className = 'questionBlock hide'
		document.getElementById('imageRTTriangle').className += ' disabledImage'
		document.getElementById('tickRTTriangle').className = 'tick'
		document.getElementById('imageRTTriangle').style.opacity = '0.3'
		this.i ++
		this.replaceDivs()
	},
	...
	validateRTTriangleInputs: function() {
		var options1, options2, options3, options4
		var a, b, c, d
		...
		if ( a !== '2')
			alert('Incorrect value of input variables(arguments). Calculating the area of a right angled triangle requires the length of the two different adjcent sides of the triangle. Try again.')
		else if ( b !== 'float' )
	...
	displayFunctionForRTTriangle: function() {
		document.getElementById('functionRTTriangle').innerHTML += 'float area_rttri (float a,float b)<br>{<br> &emsp; float area = 0.5*a*b;<br> &emsp; return area;<br>}'
	},
	...
	resetToInitialState: function() {
		...
		document.getElementById('imageRTTriangle').className = 'imageRTTriangle'
		...
		document.getElementById('tickRTTriangle').className = 'tick hide'
		...
		document.getElementById('functionRTTriangle').innerHTML = ''
	...
	calculateArea: function() {
		...
		for ( i = 0 ; i < elements.length ; i ++ ) {
			...
			else if ( parameter.length === 2 && functionCall === 'area_rttri()' && isNaN(parameter[0]) === false && isNaN(parameter[1]) === false )
				this.area_rttri(parameter, parsedValue)
			else
		 	alert( 'Incorrect function call at line ' + String( i + 1 ) )
		}	
